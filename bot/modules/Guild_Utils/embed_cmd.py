...
async def update_preview(ctx):
    try:
        await ctx.bot.edit_message(
            ctx.objs["embed_preview_msg"], embed=ctx.objs["embed_embed"]
        )
...
async def set_thumbnail(ctx):
    new_msg = (
        "Please submit the thumbnail image! (Must be a valid URL, type c to cancel.)"
    )
    await ctx.bot.edit_message(ctx.objs["menu"]["msg"], new_msg)
...
async def set_footer(ctx):
    ...
    output = await ctx.wait_for_string()
    if output is not None:
        ctx.objs["embed_embed"].set_footer(
            text=output, icon_url=ctx.objs["embed_embed"].footer.icon_url
        )
...
async def set_footer_icon(ctx):
    ...
    if output is not None:
        ctx.objs["embed_embed"].set_footer(
            text=ctx.objs["embed_embed"].footer.text, icon_url=output
        )
...
async def save_and_exit(ctx):
    ...
    if embed_name is not None:
        if embed_name in server_embeds:
            resp = await ctx.ask(
                "Are you sure you want to overwrite the stored embed `{}`?".format(
                    embed_name
                )
            )
        ...
        try:
            await ctx.data.servers_long.set(
                ctx.server.id, "server_embeds", server_embeds
            )
            await ctx.reply(
                "The embed has been saved!\n To view the embed use `{prefix}embed {name}`, and reopen the Embed Editor with `{prefix}editembed {name}`.".format(
                    prefix=ctx.used_prefix, name=embed_name
                )
            )
        except Exception:
            await ctx.reply(
...
async def ask_for_field(ctx):
    ...
    inline = await ctx.ask(
        "Should the field be displayed inline?", use_msg=ctx.objs["menu"]["msg"]
    )
    if inline is None:
        return None
...
async def add_field(ctx):
    ...
    if field is not None:
        ctx.objs["embed_embed"].add_field(
...
async def remove_field(ctx):
    ...
    if not fields:
        await ctx.bot.edit_message(
            ctx.objs["menu"]["msg"],
            "There are no fields to remove! Type anything to continue.",
        )
...
async def edit_field(ctx):
    ...
    if not fields:
        await ctx.bot.edit_message(
            ctx.objs["menu"]["msg"],
            "There are no fields to edit! Type anything to continue.",
        )
        await ctx.wait_for_string()
        return
    ...
    field = await ask_for_field(ctx)
    if field is not None:
        ctx.objs["embed_embed"].set_field_at(
            result, name=field["name"], value=field["value"], inline=field["inline"]
        )
...
async def get_server_embed(ctx, emb_name):
    ...
    if not server_embeds:
        await ctx.reply(
            "There are no saved embeds in this server! Moderators may create embeds using `{}buildembed`".format(
                ctx.used_prefix
            )
        )
        return
    if emb_name == "":
        ...
        await ctx.reply(
            "No embed exists with this name! Use `{}embed` to see a list of embeds.".format(
                ctx.used_prefix
            )
        )
        return
...
async def cmd_embedinto(ctx):
    ...
    fetch_embed = await get_server_embed(
        ctx, ctx.arg_str[len(ctx.params[0]) :].strip() if len(ctx.params) > 1 else ""
    )
    if fetch_embed is None:
        return

    out_msg = await ctx.reply(
        "Searching for message, please wait {}".format(
    ...
    if not message:
        ...
        await ctx.bot.edit_message(
            out_msg, "I can't embed into someone else's message!"
        )
    else:
        try:
            await ctx.bot.edit_message(message, embed=fetch_embed[1])
            await ctx.bot.edit_message(
                out_msg, "Successfully embedded into the message!"
            )
        except Exception:
            await ctx.bot.edit_message(
                out_msg, "An unknown error occurred while embedding!"
            )
