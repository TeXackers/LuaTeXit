...
async def cmd_score(ctx: Context, flags: dict):
    ...
    Description:
        Scores a complete 14-tile mahjong hand per Hong Kong rules (largely). Tiles are written in compact notation (`m`/`p`/`s`/`z` suffixes). The last tile listed is treated as your winning tile. Brackets are used for melds, i.e. `[123m]` for chow/pung/kong, `(1111z)` for a closed kong. See also `{prefix}help riichi`.
    Flags::
        seat: Your seat wind (east/south/west/north). Default `east`.
        round : The round wind. Default `east`.
        zimo: Score as a self-draw win.
    ...
    Scoring::
        half: Uses half-spice scoring. Default None (full-spice)
    Examples``:
    ...
    try:
        result = compute_score(
            ...
            spice="half" if flags["half"] else "full",
        )
    ...
    footer = (
        f"Winning tile: {tile_str(emojis_by_name, result.winning_tile)} | "
        f"Seat: {tile_str(emojis_by_name, seat_wind)} | Round: {tile_str(emojis_by_name, round_wind)}"
        f"{' | ⨉1.5 (自摸)' if flags['zimo'] else ''}"  # HK rule 1.5x
        f"{' | ½🌶️' if flags['half'] else ' | 🌶️'}"  # scoring spicyness
    )
