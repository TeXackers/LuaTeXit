...
continents = [
    {"name": name, "countries": countries} for name, countries in cont_dict.items()
]
...
def gen_tz_strings(tzlist):
    ...
    block_strs = [
        [
            "{0[0]:^{max_len}} {0[1]:^10}".format(tzpair, max_len=max_block_lens[i])
            for tzpair in tzblock
        ]
...
async def tz_lookup(ctx, search_str):
    ...
    searchlist = [
        (
            tz,
            "{} {} {}".format(
                tz, get_time(tz).strftime("%I:%M%p"), get_time(tz).strftime("%H:%M")
            ).lower(),
        )
    ...
    if ":" in search_str:
        ...
        if not options:
            ...
            if search_str[-1].isdigit():
                ...
                search_str = (
                    search_str[:-3] + str(int(search_str[-3]) + 1) + search_str[-2:]
                )
                options = [tz for tz, tzstr in searchlist if search_str in tzstr]
    else:
...
def get_timestr(tz, brief=False):
    ...
    """
    format_str = (
        "**%H:%M, %d/%m/%Y**" if brief else "**%I:%M %p (%Z)** on **%a, %d/%m/%Y**"
    )
...
async def cmd_time(ctx, flags):
    ...
    if flags["set"]:
        ...
        if tz:
            ...
            if ctx.args:
                methods.append(
                    "Try using our interactive timezone picker with `{prefix}ti --set`"
                )
            else:
                methods.append(
            ...
            methods.append(
                "Find your timezone in the complete list with `{prefix}ti --list`"
            )
            methods.append(
            ...
            methods.append(
                "Or join our [support server]({support}) and ask one of our friendly support team!"
            )

            desc = ("\n".join(methods)).format(
                prefix=prefix, support=ctx.client.app_info["support_guild"]
            )
            embed = discord.Embed(
                title=setter_help, description=desc, colour=discord.Colour.green()
            )
        ...
        if not ctx.args:
            ...
            await ctx.reply(
                "Usage: `{prefix}ti --at <timezone>`, e.g. `{prefix}ti --at Melbourne`".format(
                    prefix=prefix
                )
            )
        else:
        ...
        await ctx.offer_delete(
            await ctx.pager(
                paginate_list(gen_tz_strings(all_timezones), title="Timezone list")
            )
        )
    elif flags["brief"]:
        brief = 1 - brief
        time_data.upsert(
            constraint="userid", userid=ctx.author.id, brief_display=bool(brief)
        )
        await ctx.reply(
            "Your clock is now more compact."
            if brief
            else "Your clock is now more verbose."
        )
    elif flags["reset"]:
        ...
        # Consider Message references for selecting a user
            ...
            user = (
                (await ctx.find_member(ctx.args, interactive=True))
                if ctx.args
                else ctx.author
            )
        if user is None:
            ...
            if not tz:
                ...
                timestr = get_timestr(tz, brief=brief)
                tdiffstr = (
                    await time_diff(ctx, tz, auth_tz, brief=brief)
                    if user != ctx.author
                    else ""
                )
                msg = "The current time for **{}** is {}{}".format(
                    user.name, timestr, tdiffstr or "."
                )
