...
class tableSchema:
    ...
    def for_mysql(self):
        primary_keys = ",".join(
            column.name for column in self.columns if column.primary
        )
        foreign_keys = ",\n\t".join(
            foreign_key.for_mysql for foreign_key in self.foreign_keys
        )
        ...
        return self.mysql_formatstr.format(
            ...
            primary_str=",\n\tPRIMARY KEY ({})".format(primary_keys)
            if primary_keys
            else "",
    ...
    def for_sqlite(self):
        if not any(column.autoincrement for column in self.columns):
            ...
            primary_keys = ""
        foreign_keys = ",\n\t".join(
            foreign_key.for_sqlite for foreign_key in self.foreign_keys
        )
...
def schema_generator(table_name, *columns, add_timestamp=True, add_app=False):
    ...
    mysql_schema = table_formatstr.format(
        table=table_name, col_strs=mysql_col_strs, primary_str=primary_key_str
    )

    # Generate sqlite schema
    sqlite_col_strs = ",\n\t".join(column.for_sqlite for column in columns)
    sqlite_schema = table_formatstr.format(
        table=table_name, col_strs=sqlite_col_strs, primary_str=primary_key_str
    )
