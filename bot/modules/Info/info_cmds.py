...
async def get_server_avatar(ctx, gid, uid):
    ...
    url = (
        "https://cdn.discordapp.com/guilds/{}/users/{}/avatars/{}.{}?size=1024".format(
...
async def get_user_banner(ctx, uid):
    ...
    url = "https://cdn.discordapp.com/banners/{}/{}.{}?size=2048".format(
        uid, res["banner"], filetype
    )
    return url
...
async def cmd_roleinfo(ctx: Context):
    ...
    if not ctx.args:
        await ctx.pager(
            paginate_list(
                [role.name for role in reversed(guild_roles)], title="Guild roles"
            )
        )
        return
...
async def cmd_userinfo(ctx: Context):
    ...
    name = "{} {}".format(
        user, ctx.client.conf.emojis.getemoji("bot") if user.bot else ""
    )
    ...
    presence = "{} {}".format(
        ctx.client.conf.emojis.getemoji(user.status.name), statusnames[user.status]
    )
...
async def cmd_guildinfo(ctx: Context, flags):
    ...
    channels = "{} text, {} voice, {} categor{} | {} total".format(
    ...
    emb_fields = [
        ...
        ("Member Status by Device", devicestatus, 0),
    ]
...
async def cmd_channelinfo(ctx: Context, flags):
    ...
    valid = [
        ch
        for ch in gch
        if (ch.permissions_for(user).read_messages)
        and (ch.permissions_for(me).read_messages)
    ]
    ch = ctx.ch
    if ctx.args:
    ...
    elif (isinstance(ch, discord.VoiceChannel)) or (
    ...
    if isinstance(ch, discord.CategoryChannel):
        ...
        if valid:
            chlist = ", ".join(
                chan.mention if isinstance(chan, discord.TextChannel) else chan.name
                for chan in valid
            )
